...
function parseArgs() {
    ...
    if (args.length < 1) {
        console.error('Usage: node color.js [base_colors in hex with #] [number of gens] [d|l|b] [percentage]');
        process.exit(1);
    }
    const baseColors = args[0].split('#').filter(Boolean).map(hex => '#' + hex);
    const number = args.length > 1 ? parseInt(args[1], 10) : 1;
    const mode = args.length > 2 ? args[2] : 'd';
    const percentage = args.length > 3 ? parseFloat(args[3]) : 0.15;
    return { baseColors, number, mode, percentage };
}
...
// Function to generate colors based on mode
function generateColors(baseColorHex, number, mode, percentage) {
    const [r, g, b, a] = hexToRgba(baseColorHex);
    var colors
    ...
    switch (mode) {
        case 'd':
            for (let i = 0; i < number; i++) {
                colors.push(Colour.getDarkerColour(r, g, b, a, percentage));
                percentage += percentage;
            }
            break;
        case 'l':
            for (let i = 0; i < number; i++) {
                colors.push(Colour.getBrighterColour(r, g, b, a, percentage));
                percentage += percentage;
            }
            break;
        case 'b':
            for (let i = 0; i < number; i++) {
                const darker = Colour.getDarkerColour(r, g, b, a, percentage);
                const lighter = Colour.getBrighterColour(r, g, b, a, percentage);
                colors.push(darker);
                colors.unshift(lighter);
                percentage += percentage;
            }
            break;
        default:
            console.error('Invalid mode. Use "d" for dark, "l" for light, or "b" for balanced.');
            process.exit(1);
    }
    return colors;
}
// Main function
function main() {
    const { baseColors, number, mode, percentage } = parseArgs();
    var c_str = ""
    baseColors.forEach(baseColorHex => {
        const colors = generateColors(baseColorHex, number, mode, percentage);
        const hexColors = colors.map(color => rgbaToHex(...color));
        console.log(`Base color ${baseColorHex}:`, hexColors);
        c_str += hexColors.join(",")
    });
    console.log(c_str)
}
