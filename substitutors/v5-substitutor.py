...
def process_json_template(template_path, toml_files):
    ...
    for toml_path in toml_files:
        ...
        if 'stlye-overrides' in toml_data:
            apply_overrides(processed_theme['style'], toml_data, toml_data['overrides-wild'])
...
def apply_direct_overrides(theme, toml_data, overrides):
    ...
    for override_key, override_value in overrides.items():
        ...
        while i < len(parts):
            ...
            if part.isdigit():
                part = int(part)
            # print(part, part in current, str(current[part])[:150])
            next = current[part]
...
def reverse_process(template_path, final_theme_path, threshold=1):
    ...
    matching_templates = len(template['themes']) != len(final_theme['themes'])
    themes = template['themes']

    paths = []

    for i, theme in enumerate(final_theme['themes']):
        name, Data = extract_variables(matching_templates and themes[i] or themes[0], theme)
        output_toml_path = final_theme_path.with_name(f'{name}.toml')
        variables, overrides, not_done, var_map = Data["variables"], Data["overrides"], Data["not_done"], Data["var_map"]

        color_counts, variables, overrides = handle_colors(variables, overrides, var_map, threshold)
        generate_toml(variables, overrides, color_counts, var_map, output_toml_path)

        paths.append(output_toml_path)

    return paths

def extract_variables(template, final_theme, Data={}, prefix=""):
    if not Data:
    ...
    for key, value in final_theme.items():
        ...
        if isinstance(value, dict) and isinstance(template.get(key), dict):
            extract_variables(template[key], value, Data, full_key)
        elif isinstance(value, list) and isinstance(template.get(key), list):
            for i, item in enumerate(value):
                if isinstance(item, dict):
                    extract_variables(template[key][i], item, Data, f"{full_key}.{i}")
                else:
                    not_done[full_key] = (item, template[key][i])
        else:
            vars = parse_variable(template.get(key, None))
    ...
    for key, value in final_theme.items():
        ...
        templ_val = template.get(key, None)
        if isinstance(templ_val, str) and templ_val.startswith("$"):
            full_key = f"{prefix}.[{key}]" if prefix and "." in key else full_key
...
def generate_toml(variables, overrides, color_map, var_map, output_path):
    toml_content = ""
    sub_section_regex = r'(.+)\.([^.]+)$'
    sub_sections = {}
    s = lambda x: f"\n[{x}]\n"
    ...
    def get_original_color(var_name, value):
        ...
        colors = set(
            filter(lambda c: is_hex_color(c) and color_map.get(get_base_color(c)) == value,
                [val for val in val_map.values()]))
